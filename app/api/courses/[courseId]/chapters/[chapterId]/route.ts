...
export async function GET(
  ...
  try {
    ...
    const isFree = chapter.course.price === 0 || chapter.course.price === null;
    if (!isOwner && !chapter.isFree && !isFree) {
    ...
    const certStatus = await getCourseCertificateStatus(userId, courseId);
