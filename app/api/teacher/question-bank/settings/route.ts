...
export async function GET() {
  try {
    ...
    if (user?.role !== "TEACHER") return new NextResponse("Forbidden", { status: 403 });

    const settings = await getQuestionBankSettings();
    return NextResponse.json(settings);
  } catch (error) {
    console.error("[TEACHER_QUESTION_BANK_SETTINGS_GET]", error);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error("[TEACHER_QUESTION_BANK_SETTINGS_PUT]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
