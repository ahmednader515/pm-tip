...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // If already logged in, redirect to device-conflict page
      if (statusData.isAlreadyLoggedIn) {
        router.push(`/device-conflict?phoneNumber=${encodeURIComponent(formData.phoneNumber)}`);
        return;
      }
      ...
      if (result?.error) {
        if (result.error === "UserAlreadyLoggedIn") {
          router.push(`/device-conflict?phoneNumber=${encodeURIComponent(formData.phoneNumber)}`);
          return;
        }
